...
import { isEqualTo, isSha1Hash, isString, JSONObject, optional, Sha1Hash, sleepMsec, _validateObject } from '../../types/kacheryTypes';
...
class TaskManager {
    ...
    constructor(private clientChannel: PubsubChannel | undefined, private objectStorageClient: ObjectStorageClient | null, private googleSignInClient: GoogleSignInClient | undefined) {
        this.#onPublishToTaskQueue = (msg: TaskQueueMessage) => {
            const msg2 = googleSignInClient ? {...msg, idToken: googleSignInClient.idToken} : msg
            clientChannel && clientChannel.publish({data: msg2 as any as JSONObject})
        }
        this._start()
    }
